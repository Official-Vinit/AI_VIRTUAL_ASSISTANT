...
function SignUp() {
    ...
    return (
        <div className='w-full h-screen bg-gray-50 flex justify-center items-center'>
            <form className='w-[90%] max-w-[400px] bg-white border border-gray-200 shadow-sm flex flex-col items-center justify-center gap-[20px] p-[30px] rounded' onSubmit={handleSignUp}>
                <h1 className='text-gray-800 text-[24px] font-semibold mb-[10px]'>Register to <span className='text-blue-600'>Virtual Assistant</span></h1>
                <input type="text" placeholder='Enter your Name' className='w-full h-[50px] outline-none border border-gray-300 bg-white text-gray-800 placeholder-gray-400 px-[15px] rounded text-[16px]' required onChange={(e) => setName(e.target.value)} value={name} />
                ...
                <button className='w-full h-[50px] mt-[10px] text-white font-semibold bg-blue-600 hover:bg-blue-700 rounded text-[16px]' disabled={loading}>{loading ? "Loading..." : "Sign Up"}</button>

                <p className='text-gray-600 text-[14px] cursor-pointer mt-2' onClick={() => navigate("/signin")}>Already have an account? <span className='text-blue-600'>Sign In</span></p>
            </form>
        </div>
    )
}
